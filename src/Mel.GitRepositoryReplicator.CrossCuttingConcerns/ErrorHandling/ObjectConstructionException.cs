...
public class ObjectConstructionException : Exception
{
		...
		object?[] constructionMethodParameters)
	{
		...
		{
			sb.Append($"with the following parameter: {constructionMethodParameters.First().Render()} ");
		}
		else if (constructionMethodParameters.Length > 1)
		{
			sb.Append($"with the following parameters: [{string.Join(", ", constructionMethodParameters.Select(p => p.Render()))}] ");
		}
		...
		string callerMethodName = "")
	{
		...
		sb.Append($"{memberUnderConstruction} cannot accept value {invalidValue.Render()} ");
