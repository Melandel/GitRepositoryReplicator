...
class CodeRepositoryPersister : ICodeRepositoryPersister
{
	...
	void ReplaceCodeBase(Application.FolderPath targetRepositoryPath, CodeBase codeBase)
	{
		_filesystem.ClearFolderExceptTheDotGitFolder(targetRepositoryPath);
		_filesystem.Write(codeBase, targetRepositoryPath);
	}

	void Commit(Application.FolderPath targetRepositoryPath, CodeBaseEvolutionStepDescription stepDescription)
	{
