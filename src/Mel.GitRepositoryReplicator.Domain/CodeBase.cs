		...
		Language targetRepositoryDocLanguage)
	{
		...
		foreach (var filepath in filepaths)
		{
			_files[filepath] = _files[filepath]
				...
				.AdaptDocumentationLanguageTo(targetRepositoryMessagesLanguage);

			var adaptedFilePath = filepath.Duplicate().AdaptRootNamespaceTo(targetRepositoryRootNamespace);
			if (adaptedFilePath != filepath)
			{
				_files[adaptedFilePath] = _files[filepath];
